...
from aind_behavior_dynamic_routing_bonsai.data_mappers import AindRigDataMapper, AindSessionDataMapper

sys.path.append(".")
from examples.rig import rig
from examples.session import session
from examples.task_logic import task_logic
...
class TestAindSessionDataMapper(unittest.TestCase):
    ...
    @patch("aind_behavior_dynamic_routing_bonsai.data_mappers.AindSessionDataMapper._map")
...
class TestAindRigDataMapper(unittest.TestCase):
    ...
    @patch("aind_behavior_dynamic_routing_bonsai.data_mappers.AindRigDataMapper._map")
    def test_mock_map(self, mock_map):
        mock_map.return_value = MagicMock()
        result = self.mapper.map()
        self.assertIsNotNone(result)
